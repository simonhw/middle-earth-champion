...
def validate_name(message, parameter):
    ...
    Checks that the user input does not contain any specific symbols.
...
def push_details(list):
    '''
    Takes in a list of details generated by the Classes and pushes the list to
...
def get_worksheet(worksheet):
    ...
    If the list contains no data entries apart from the column headings, it
...
def admin_access():
    '''
    Runs the necessary processes that verify admin status and allows access
